...
## Creating a new dnevnik project

To create a new dnevnik project, we use `mix`

    $ mix new blog_name

We then modify our dependencies within `mix.exs` to include dnevnik.

    defp deps do
      [{ :dnevnik, "~> 0.1.0" }]
    end

Next we need to download dnevnik and compile it

    $ mix deps.get
    $ mix deps.compile

Now for the fun stuff, we can setup our dnevnik project

    $ mix dnevnik setup

We can build our dnevnik project now. 
